@extends('master')
@section('content')
        {!! Form::open(['class' =>'form-sample','route' => 'loan.store','method' => 'POST', 'enctype' => 'multipart/form-data']) !!}
        <div class="row">
            <div class="col-md-3"></div>
            <div class="col-md-6">
                <div class="page-header" id="bannerClose"><h3 class="page-title"><span class="page-title-icon bg-gradient-primary text-white mr-2"><i class="mdi mdi-account"></i></span>Loan</h3></div>
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control" name="loan_period" id="loan_period" >
                                    ...
                                    @foreach($loan_periods as $loan_period)
                                        <option value="{{$loan_period->id}}">{{$loan_period->loan_period}}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-group">
                                <label for="loan_amount">Loan Amount</label>
                                <input type="number" class="form-control" id="loan_amount" name="loan_amount" readonly required>
                            </div>
                            <div class="form-group">
                                <label for="interest">Interest (%)</label>
                                <input type="number" class="form-control" id="interest" name="interest" readonly required>
                            </div>
                            <div class="form-group">
                                <label for="net_amount">Net Amount</label>
                                <input type="number" class="form-control" id="net_amount" name="net_amount" readonly required>
                            </div>
                            <div class="form-group">
                                <label for="repaid_p_m">Repaid P/M</label>
                                <input type="number" class="form-control" id="repaid_p_m" name="repaid_p_m" readonly required>
                            </div>
                            <div class="form-group">
                                <label for="total_paid">Total Paid</label>
                                <input type="number" class="form-control" id="total_paid" name="total_paid" placeholder="Total Paid">
                            </div>
                            <div class="form-group">
